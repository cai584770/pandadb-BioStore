...
object ReStoreSequence {
   ...
   * @return sequence
   */
  def from2bit(binaryArray: Array[Byte], supplementaryInformation: Map[String, List[(Any, Any)]], bioSequenceType: BioSequenceType = DNA): String = {
...
//    println(binaryString)
  ...
  def convertFromBinaryArray(bytes: Array[Byte],bioSequenceType:BioSequenceType): String = {
    val conversionMap = bioSequenceType match {
      case DNA => Map("00" -> 'A', "01" -> 'G', "10" -> 'C', "11" -> 'T')
      case RNA => Map("00" -> 'A', "01" -> 'G', "10" -> 'C', "11" -> 'U')
